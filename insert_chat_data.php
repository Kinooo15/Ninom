...
if ($user_type === "admin") {
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['message']) && isset($_POST['receiver'])) {
        $newMessage = mysqli_real_escape_string($dlink, $_POST['message']);
        $recipient = mysqli_real_escape_string($dlink, $_POST['receiver']);

        $query = "INSERT INTO messages VALUES(1, $recipient, '$newMessage', NOW())";
        $insertResult = mysqli_query($dlink, $query);

        if (!$insertResult) {
            echo "Error inserting chat message: " . mysqli_error($dlink);
        }
    }
} else {
...
if ($result) {
    ...
    echo "Error retrieving chat messages: " . mysqli_error($dlink);
}
