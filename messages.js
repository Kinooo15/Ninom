...
function generateMessageHTML(sender, message, timestamp) {
    ...
    <div>${message}</div>
    <div>${timestamp}</div>
    <button class="reply-button" data-recipient="${sender}" style="background-color: transparent; font-weight: bold; color: gray; border: 0;" onmouseover="this.style.color='red'" onmouseout="this.style.color='gray'" onclick="this.style.color='red'">Reply</button>
    `;
